...
let CoffeesService = class CoffeesService {
    ...
    async create(createCoffeDto) {
        const flavors = await Promise.all(createCoffeDto.flavors.map(name => this.preloadFlavorByName(name)));
        const coffee = this.coffeeRepository.create(Object.assign(Object.assign({}, createCoffeDto), { flavors }));
        return this.coffeeRepository.save(coffee);
    }
    async update(id, updateCoffeDto) {
        const flavors = updateCoffeDto.flavors && (await Promise.all(updateCoffeDto.flavors.map(name => this.preloadFlavorByName(name))));
        const coffee = await this.coffeeRepository.preload(Object.assign(Object.assign({ id: +id }, updateCoffeDto), { flavors }));
        if (!coffee) {
    ...
    async recommendCoffee(coffee) {
        ...
        try {
            ...
            const recommendEvent = new event_entity_1.Evento();
